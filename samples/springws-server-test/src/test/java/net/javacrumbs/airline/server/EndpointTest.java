...
public class EndpointTest {
	...
	public void testValidateResponse() throws Exception {
		client.sendRequest(withMessage("request1.xml")).andExpect(noFault()).andExpect(validPayload(resource("xsd/messages.xsd"),resource("xsd/types.xsd")));
	}
	@Test
	public void testAssertXPath() throws Exception {
		client.sendRequest(withMessage("request1.xml")).andExpect(noFault()).andExpect(xpath("//ns:from/ns:code",NS_MAP).evaluatesTo("PRG"));
	}
	...
	public void testErrorMessage() throws Exception {
		client.sendRequest(withMessage("request-error.xml")).andExpect(clientOrSenderFault("Departure and destination airport has to differ."));
	}

	@Test
	public void testResponseTemplate() throws Exception {
		client.sendRequest(withMessage("request-context-xslt.xml").withParameter("serviceClass","first")).andExpect(message("response-context-xslt.xml").withParameter("serviceClass", "first"));
	}
}